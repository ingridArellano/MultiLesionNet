...
def laplacian_filter(image, sigma = 2.0) :
    laplacian = sitk.LaplacianRecursiveGaussianImageFilter()
    laplacian.SetSigma(sigma)
    return  laplacian.Execute(image)

def discreteGaussian_filter(image):
    discreteG = sitk.DiscreteGaussianImageFilter()
    discreteG.SetVariance(2.0)
    return discreteG.Execute(image)

def bilateral_filter(image, sigmaD, sigmaR) :
    '''
    ...
    domainSigma: double
        controls the distance by which neighboring pixels contribute to a pixel's value
    rangeSigma: double
        controls the difference in intensity between neighboring pixels that contribute to a pixel's value
    '''

    bilateral = sitk.BilateralImageFilter()
    bilateral.SetDomainSigma(sigmaD)
    bilateral.SetRangeSigma(sigmaR)

    return bilateral.Execute(image)
...
def bilateral_smooth(image, domainSigma = 2.0, rangeSigma=50.0):
    '''
    Apply a bilateral smoothing to the input image
    ...
    '''
    bilateral = sitk.BilateralImageFilter()
    bilateral.SetDomainSigma(domainSigma)
    bilateral.SetRangeSigma(rangeSigma)
    return  bilateral.Execute(image)
